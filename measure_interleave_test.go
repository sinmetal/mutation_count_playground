...
func TestMeasureInterleave_Insert(t *testing.T) {
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			mu, err := createInterleaveInsertMutation(tt.normalColumnCount, tt.addColumn, tt.rowCount)
			if err != nil {
				t.Fatal(err)
			}
...
// createInterleaveInsertMutation is Interleave Table の Insert Mutationを作成する
...
func createInterleaveInsertMutation(normalColumnCount int, addColumn map[string]interface{}, rowCount int) ([]*spanner.Mutation, error) {
